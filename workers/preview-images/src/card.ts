...
import { buildAverageChartPoints, buildChartPresentation, buildCourseChartDomain, buildSectionActivityTimeline, buildSectionChartPoints, getEnrollmentScaleMax, getMajorMilestones, getSectionTypeName } from '../../../assets/website/src/chartMapping.mjs';
...
function graph(state: PreviewState): string {
  ...
  const visibleMilestones: Milestone[] = getMajorMilestones(state.milestones ?? []);
  const presentation = buildChartPresentation({
    points,
    domain,
    milestones: visibleMilestones,
    phaseMilestones: state.milestones ?? [],
    majorMilestones: visibleMilestones,
    mode: 'phased',
  });
  ...
  const milestones = visibleMilestones.flatMap((milestone, index) => {
    ...
    const label = milestone.priority ? `Priority ${milestone.priority}` : milestone.label;
    ...
    return `<g class="milestone"><line x1="${x.toFixed(1)}" y1="38" x2="${x.toFixed(1)}" y2="190" style="stroke:${escapeHtml(color)}"/><text x="${textX.toFixed(1)}" y="22" text-anchor="${anchor}" style="fill:${escapeHtml(color)}">${escapeHtml(label)}</text></g>`;
  }).join('');
  const visibleSet = new Set(visibleMilestones);
  const minorMilestones = (state.milestones ?? []).flatMap((milestone) => {
    if (visibleSet.has(milestone) || !milestone.priority) return [];
    const parsed = Date.parse(milestone.time.replace(' ', 'T'));
    if (!Number.isFinite(parsed)) return [];
    const x = xFor(presentation.mapTime(parsed));
    return `<g class="minor-milestone"><title>Priority ${escapeHtml(milestone.priority)}, ${escapeHtml(milestone.label)}</title><line x1="${x.toFixed(1)}" y1="52" x2="${x.toFixed(1)}" y2="190" style="stroke:${escapeHtml(milestone.color)}"/></g>`;
  }).join('');
  ...
  return `<svg class="graph" viewBox="0 0 1120 210" role="img" aria-label="Milestone-phased enrollment and capacity history" data-segments="${intervalIndices.length}"><line class="axis" x1="40" y1="190" x2="1080" y2="190"/>${minorMilestones}${milestones}${annotation}${capacityPaths}${enrollmentPaths}<text class="series-label" x="40" y="207">${escapeHtml(seriesLabel)}, milestone view</text></svg>`;
}
...
function formatNextMilestone(next: { label: string; time: string } | null | undefined): string {
  if (!next) return '';
  ...
  return `Next: ${String(next.label)} — ${when}`;
}
...
export function renderCard(state: PreviewState): string {
  ...
  const priority = state.priority?.label ? `<span class="priority">${escapeHtml(state.priority.label)}</span>` : '';
...
*{box-sizing:border-box}html,body{margin:0;width:1200px;height:630px;overflow:hidden;background:hsl(234 45% 13%);color:hsl(0 0% 92%);font-family:'JetBrains Mono','Fira Code','SF Mono',monospace;font-variant-numeric:tabular-nums}.card{position:relative;width:100%;height:100%;padding:42px 64px;background:linear-gradient(135deg,hsl(220 45% 16%),hsl(234 45% 13%) 72%)}.mark{position:absolute;top:44px;right:64px;width:30px;height:30px;background:#ff5722}.eyebrow{color:hsl(48 100% 50%);font-size:22px;line-height:1.25;letter-spacing:.09em;text-transform:uppercase}.title{margin:14px 0 4px;max-width:980px;font-size:62px;line-height:1.04;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.subtitle{max-width:980px;color:hsl(0 0% 70%);font-size:27px;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.priority{color:hsl(48 100% 50%);font-size:17px;font-weight:700;line-height:1.25}.course-readout{width:100%;margin-top:12px;padding:9px 13px;border:1px solid hsl(220 10% 50%);background:hsl(234 45% 13% / .74);box-shadow:0 3px 10px hsl(234 45% 8% / .24);font-size:17px;line-height:1.25}.course-readout-header{display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:5px 28px;min-height:21px}.course-readout-registration{display:flex;flex-wrap:wrap;align-items:center;gap:5px 28px}.course-readout-context{color:hsl(48 100% 50%);font-size:17px;font-weight:700}.course-readout-sections{color:hsl(0 0% 70%);font-size:15px}.course-readout-values{display:flex;flex-wrap:wrap;gap:4px 24px;margin-top:3px}.course-readout-line{display:inline-flex;gap:7px;white-space:nowrap}.course-readout-label{color:hsl(0 0% 70%)}.course-readout-value{font-weight:600}.course-readout-line:last-child .course-readout-value{color:hsl(48 100% 50%)}.has-graph .graph{position:absolute;left:28px;bottom:18px;width:1144px;height:294px}.no-graph.course-card{display:flex;flex-direction:column;justify-content:center;padding-top:48px;padding-bottom:48px}.no-graph.course-card .mark{top:66px}.no-graph.course-card .title{margin-top:18px;font-size:72px}.no-graph.course-card .subtitle{font-size:31px;margin-top:7px}.no-graph.course-card .course-readout{margin-top:25px;min-height:230px;padding:22px 26px;display:flex;flex-direction:column;justify-content:center;font-size:22px}.no-graph.course-card .course-readout-header{gap:12px 40px}.no-graph.course-card .course-readout-registration{gap:12px 40px}.no-graph.course-card .priority,.no-graph.course-card .course-readout-context{font-size:21px}.no-graph.course-card .course-readout-sections{font-size:19px}.no-graph.course-card .course-readout-values{display:flex;flex-direction:column;gap:10px;margin-top:16px}.no-graph.course-card .course-readout-line{display:grid;grid-template-columns:160px minmax(0,1fr);gap:14px;white-space:normal}.no-graph.course-card .course-readout-value{text-align:right}.semester-card{padding:62px 64px}.semester-card .mark{top:64px}.semester-card .title{margin-top:26px;font-size:88px}.semester-stats{display:grid;grid-template-columns:repeat(4,1fr);margin-top:54px;border-top:1px solid hsl(240 4% 37%);border-bottom:1px solid hsl(240 4% 37%)}.semester-stats div{padding:34px 22px;text-align:right}.semester-stats div+div{border-left:1px solid hsl(240 4% 37%)}.semester-stats strong,.semester-stats span{display:block}.semester-stats strong{font-size:46px;line-height:1.1;color:hsl(0 0% 92%)}.semester-stats span{margin-top:10px;color:hsl(0 0% 70%);font-size:17px;text-transform:uppercase;letter-spacing:.05em}.semester-update{margin-top:32px;font-size:24px;line-height:1.25}.semester-registration{display:flex;flex-wrap:wrap;gap:10px 32px;align-items:center;margin-top:20px;padding:18px 22px;border:1px solid hsl(220 10% 50%);background:hsl(234 45% 13% / .55)}.semester-registration .priority,.semester-registration .course-readout-context{font-size:21px}.semester-eligible{color:hsl(0 0% 78%);font-size:20px}.graph .axis{stroke:hsl(220 10% 50%);stroke-width:1.25}.enrollment-series{fill:none;stroke:hsl(48 100% 50%);stroke-width:5;stroke-linecap:round;stroke-linejoin:round}.capacity-series{fill:none;stroke:hsl(177 56% 60%);stroke-width:3;stroke-dasharray:9 7;opacity:.96}.milestone line{stroke-width:2;stroke-dasharray:5 5;opacity:.95}.minor-milestone line{stroke-width:1.5;stroke-dasharray:3 6;opacity:.78}.milestone text,.series-label,.capacity-change{font:12px 'JetBrains Mono','Fira Code','SF Mono',monospace;letter-spacing:.04em}.series-label{fill:hsl(0 0% 76%)}.capacity-change{fill:hsl(177 56% 70%)}.removed{display:inline-block;margin-left:14px;padding:5px 8px;background:hsl(346 100% 60%);color:hsl(234 45% 13%);font-size:16px;line-height:1.25}
