...
type ChartPoint = {
  ...
  capacity?: number;
  prevCapacity?: number;
  capacityChanges?: Array<{ sectionCode: string; previousCapacity: number; capacity: number }>;
};
...
function graph(state: PreviewState): string {
  ...
  const presentation = buildChartPresentation({
    points,
    domain,
    // Preserve every observed capacity change, including observations outside milestones.
    milestones: [],
    phaseMilestones: state.milestones ?? [],
    mode: 'phased',
  });
  const visiblePoints: ChartPoint[] = presentation.visiblePoints;
  ...
  const xDomain = [...mappedDomain, ...mappedMilestones, ...presentation.xValues];
  ...
  const latestMarker = Number.isFinite(latestX)
    ...
    : '';
  const changes = visiblePoints.flatMap((point, index) => {
    if (!point.capacityChanged) return [];
    const evidence = point.capacityChanges ?? [{
      sectionCode: sectionEntries[0]?.[0] ?? '',
      previousCapacity: point.prevCapacity ?? 0,
      capacity: point.capacity ?? 0,
    }];
    return evidence.map(change => ({
      ...change,
      x: xFor(presentation.xValues[index]),
      y: yFor(presentation.capacityValues[index]),
      enrollmentY: yFor(presentation.enrollmentValues[index]),
    }));
  });
  const markers = changes.map(change => {
    const coincidesWithLatest = Number.isFinite(latestX)
      && Math.abs(change.x - xFor(latestX)) < 1
      && Math.abs(change.y - change.enrollmentY) < 1;
    return `<circle class="capacity-change-marker" cx="${change.x.toFixed(1)}" cy="${change.y.toFixed(1)}" r="${coincidesWithLatest ? 10 : 5}"${coincidesWithLatest ? ' style="fill:none;stroke:hsl(177 65% 68%)"' : ''}><title>${escapeHtml(change.sectionCode)}: CAP ${change.previousCapacity}→${change.capacity}</title></circle>`;
  }).join('');
  const latestChange = changes.at(-1);
  let annotation = '';
  if (latestChange) {
    const label = `CAP ${latestChange.previousCapacity}→${latestChange.capacity}`;
    const width = label.length * 8 + 12;
    const preferredX = latestChange.x + 10 + width <= 1080
      ? latestChange.x + 10 : latestChange.x - width - 10;
    const x = Math.max(40, Math.min(1080 - width, preferredX));
    // Prefer the side away from enrollment; reserve the milestone label band.
    const preferredY = latestChange.enrollmentY >= latestChange.y
      ? latestChange.y - 30 : latestChange.y + 10;
    const y = Math.max(38, Math.min(166, preferredY));
    annotation = `<g><rect class="capacity-change-label" x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${width}" height="22" rx="3"/><text class="capacity-change" x="${(x + 6).toFixed(1)}" y="${(y + 15).toFixed(1)}">${label}</text></g>`;
  }

  return `<svg class="graph" viewBox="0 0 1120 210" role="img" aria-label="Equally spaced milestone enrollment and capacity history" data-segments="${intervalIndices.length}"><line class="axis" x1="40" y1="190" x2="1080" y2="190"/>${milestones}${capacityPaths}${enrollmentPaths}${markers}${latestMarker}${annotation}<text class="series-label" x="40" y="207">${escapeHtml(seriesLabel)}, milestone view</text></svg>`;
}
...
function courseSectionSummary(state: PreviewState): string {
  ...
  return `${sectionCount} sections: ${types.join(' and ')}`;
}
